...
def movie1():
    ...
    for i in range(10):
        ...
        cvote=movies['vote_count'].iloc[i]
        avote=movies['vote_average'].iloc[i]
        data = {
            'title': title,
            'id':str(id),
            'score':str(score),
...
def movies2(title):
    ...
    #유사도 테이블에서 인덱스 번째 유사도 값 가져오기
    sim_scores=sim[idx]
    #영화인덱스번호, 유사도값을 배열로 생성
    ...
    for i in range(12):
        ...
        data={'etitle':etitle, 'image':image, 'title':title}
        json.append(data)
